...
for i in range(14,len(percents)):
    ...
    recent_percents = recent_percents[-15:-1]
    recent_truths = truths[0:i+1]
    recent_truths = recent_truths[-15:-1]
...
valuevec = pd.DataFrame(valuevec,index = comb.index,columns = ['Strat Val'])
actionvec = pd.DataFrame(actionvec,index = comb.index,columns = ['Action'])
comb = pd.concat([comb,valuevec,actionvec], axis=1)
print(comb)
...
'''
for date in buy_dates[2:]:
    shifted_date = comb.index[comb.index.get_loc(date) - 2]  # Shift back by 2
    plt.axvline(x = shifted_date,color='green')
    #plt.scatter(x=shifted_date, y=comb.loc[date, title], color='green', marker='x', s=7,zorder= 2)
...
for date in sell_dates[2:]:
    shifted_date = comb.index[comb.index.get_loc(date) - 2]  # Shift back by 2
    plt.axvline(x = shifted_date,color='red')
    #plt.scatter(x=shifted_date, y=comb.loc[date, title] , color='red', marker='s', s=7,zorder= 2)
